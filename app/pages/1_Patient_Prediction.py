...
st.caption("Enter patient data to get AI-powered risk assessment with SHAP explanations")
...
def show_results(risk_level, confidence, feature_cols, X_scaled, model, risk_class,
    ...
    with r1:
        st.metric("Risk Level", f"{risk_level} Risk")
        st.progress(confidence[risk_level], text=f"{confidence[risk_level]:.1%} confidence")
    with r2:
        ...
        fig.update_layout(height=150, margin=dict(l=0, r=10, t=4, b=4),
    ...
    with r3:
        ...
        fig.update_layout(height=150, margin=dict(l=15, r=15, t=8, b=0), paper_bgcolor="white")
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
    ...
    with col_shap:
        ...
        try:
            ...
            fig.update_layout(
                height=max(250, len(feature_cols) * 24), margin=dict(l=0, r=30, t=4, b=4),
                plot_bgcolor="white", paper_bgcolor="white",
                xaxis=dict(showgrid=True, gridcolor="#F1F5F9", zeroline=True, zerolinecolor="#CBD5E1", tickfont=dict(size=9)),
        ...
        try:
            ...
            if sv is not None:
                ...
                if alerts:
                    for a in alerts:
                        if a["severity"] == "high":
                            st.error(f"**{a['feature']}**: {a['message']}")
                        else:
                            st.success(f"**{a['feature']}**: {a['message']}")
                else:
...
# ── Diabetes ─────────────────────────────────────────────────────────────────
if dataset == "Diabetes":
    st.subheader("Patient Data — Diabetes")
...
# ── Heart Disease ────────────────────────────────────────────────────────────
    ...
    st.subheader("Patient Data — Heart Disease")
    c1, c2, c3, c4 = st.columns(4)
    with c1:
